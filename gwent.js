...
class ControllerNetwork {
	...
	async startTurn(player) {
		// Ничего не делаем, ждем сетевого события от соперника
	}
...
var localDeckData = null;
var originalRandom = Math.random;

// Сеяный генератор Mulberry32
function setupSeedableRandom(seed) {
...
class Player {
	...
	passRound(){
		...
		if (isMultiplayer && this === player_me) {
			socket.emit('game_action', { type: 'PASS' });
		}
		this.endTurn();
	}
	...
	async activateLeader() {
		...
		if (isMultiplayer && this === player_me) {
			socket.emit('game_action', { type: 'LEADER' });
		}
...
class Deck extends CardContainer {
	...
	swap(container, card){
		if (!card)
			return;
		if (isMultiplayer) {
			let removedCard = container.removeCard(card);
...
class HandAI extends CardContainer {
	...
	// Override: insert in sorted order (matching Hand), so card indices stay in sync for multiplayer
	addCard(card) {
		if (!card) return;
	...
	resize() {
		this.counter.innerHTML = this.cards.length; 
		const topCounter = document.getElementById("op-hand-counter");
		if (topCounter) {
			topCounter.innerHTML = "Карты соперника: " + this.cards.length;
		}
	}
	...
	reset() {
		...
		const topCounter = document.getElementById("op-hand-counter");
		if (topCounter) {
			topCounter.innerHTML = "Карты соперника: 0";
		}
	}
}
...
class Hand extends CardContainer {
	...
	// Override: always insert in sorted order to keep in sync with HandAI
	addCard(card){
		const sortedIndex = this.addCardSorted(card);
...
class Game {
	...
	async coinToss(){
		if (this.firstPlayer)
			return;
		if (isMultiplayer) {
			let hostGoesFirst = (Math.random() < 0.5); 
			if (myPlayerIndex === 0) {
				this.firstPlayer = hostGoesFirst ? player_me : player_op;
			} else {
				this.firstPlayer = hostGoesFirst ? player_op : player_me;
			}
		} else {
	...
	async initialRedraw(){
		if (isMultiplayer) {
			let myRedrawsDone = false;
			let opRedrawsDone = false;
			
			const handleOpAction = (action) => {
				if (action.type === 'REDRAW_DONE') {
					opRedrawsDone = true;
					socket.off('opponent_action', handleOpAction);
				}
			};
			socket.on('opponent_action', handleOpAction);
	...
	returnToCustomization(){
		this.reset();
		if (player_me) player_me.reset();
		if (player_op) player_op.reset();
	...
	rematchGame(){
		if (isMultiplayer) {
			...
			ui.notification("Waiting for opponent...", 999999);
		} else {
			this.reset();
			player_me.reset();
			player_op.reset();
...
class UI {
	...
	initYouTube(){
		this.youtube = new Audio("sfx/music.mp3");
		this.youtube.loop = true;
		this.youtube.volume = 0.5; // Умеренная громкость
		ui.ytActive = true;
		
		if (Settings.music.isEnabled()) {
			// Попытка воспроизведения с обработкой блокировки автоплея
			this.youtube.play().catch(e => {
				console.log("Autoplay blocked. Waiting for first click...");
				const playOnInteract = () => {
					if (Settings.music.isEnabled() && this.youtube.paused) {
						this.youtube.play().catch(err => console.log(err));
					}
	...
	toggleMusic(){
		...
		const useMusic = Settings.music.isEnabled();
		if (useMusic) {
			this.toggleMusic_elem.classList.remove("fade");
			if (this.youtube && typeof this.youtube.play === "function") {
				this.youtube.play().catch(err => console.log(err));
			}
		} else {
			this.toggleMusic_elem.classList.add("fade");
			if (this.youtube && typeof this.youtube.pause === "function") {
				this.youtube.pause();
			}
		}
	}
	...
	setYouTubeEnabled(enable){
		if (this.ytActive === enable)
			return;
		if (enable && Settings.music.isEnabled()) {
			if (ui.youtube && typeof ui.youtube.play === "function") {
				ui.youtube.play().catch(err => console.log(err));
			}
		} else {
			if (ui.youtube && typeof ui.youtube.pause === "function") {
				ui.youtube.pause();
			}
		}
	...
	async selectCard(card) {
		let row = board.row.find(r => r.cards.includes(card)) || this.lastRow;
		...
		if (pCard === null || card.holder.hand.cards.includes(card)) {
			...
			if (isMultiplayer) {
				let decoyIdx = player_me.hand.cards.indexOf(pCard);
				let targetRowIdx = board.row.indexOf(row);
				let targetCardIdx = row.cards.indexOf(card);
				socket.emit('game_action', {
					type: 'PLAY_DECOY',
					decoyIndex: decoyIdx,
					targetRowIndex: targetRowIdx,
					targetCardIndex: targetCardIdx
				});
			}
	...
	async viewCardsInContainer(container, action, isViewOnly = true) {
		action = action ? action : function() {return this.cancel();};
		await this.queueCarousel(container, 1, action, () => true, false, true, undefined, isViewOnly);
	}
	...
	async queueCarousel(container, count, action, predicate, bSort, bQuit, title, isViewOnly = false){
		if (!isViewOnly && game.currPlayer === player_op) {
	...
	async waitForRowSelection(card)
	{
		...
		if (isMultiplayer && game.currPlayer === player_me && selectedRow) {
			socket.emit('game_action', { type: 'AGILE_CHOICE', rowIndex: board.row.indexOf(selectedRow) });
		}
		return selectedRow;
	}
}
...
class DeckMaker {
	...
	startNewGame(){
		...
		player_me = new Player(0, "Игрок 1", me_deck);
		player_op = new Player(1, "Игрок 2", op_deck);
	...
	uploadDeck(id, callback)
	{
		...
		fr.onload = e => {
			try {
				...
				alert("Загруженная колода имеет неверный формат!");
			}
			finally
			{
	...
	deckFromJSON(json) {
		...
		try {
			...
			AudioManager.playSFX('warning');
			alert("Не удалось прочитать загруженную колоду!");
			return;
		}
	}
	...
	loadDeck(deck, silent = true)
	{
		...
		if (card_dict[deck.leader].row !== "leader")
			warning += "'" + card_dict[deck.leader].name + "' не может быть использован в качестве лидера\n";
		if (deck.faction != card_dict[deck.leader].deck)
			warning += "Лидер '" + card_dict[deck.leader].name + "' не соответствует фракции колоды '" + deck.faction + "'.\n";
		// check if cards exist and have correct faction & count
		const cards = deck.cards.filter( c => {
			const card = card_dict[c[0]];
			if (!card) {
				warning += "ID " + c[0] + " не соответствует ни одной карте.\n";
				return false
			}
			if (![deck.faction, "neutral", "special", "weather"].includes(card.deck)) {
				warning += "'" + card.name + "' не может быть использована в колоде фракции '" + deck.faction +"'\n";
				return false;
			}
			if (card.count < c[1]) {
				warning += "Колода содержит " + c[1] + "/" + card.count + " доступных карт '" + card_dict[c[0]].name + "'\n";
				c[1] = card.count;
				return true;
			}
			return true;
...
// No-op since we use local HTML5 Audio now
...
async function handleOpponentAction(action) {
	switch (action.type) {
		...
		case 'PLAY_DECOY': {
			let decoyCard = player_op.hand.cards[action.decoyIndex];
			let targetRow = board.row[5 - action.targetRowIndex];
			let targetCard = targetRow.cards[action.targetCardIndex];

			ui.showPreviewVisuals(decoyCard);
			await sleep(1000);
			ui.hidePreview(decoyCard);

			board.toHand(targetCard, targetRow);
			await board.moveTo(decoyCard, targetRow, player_op.hand);
			player_op.endTurn();
			break;
		}
		case 'PASS': {
		...
		case 'REDRAW_CARD': {
			let card = player_op.hand.cards[action.index];
			await player_op.deck.swap(player_op.hand, card);
			break;
		}
		...
		case 'SCOIA_CHOICE': {
			if (player_op.controller.pendingScoiaResolve) {
				player_op.controller.pendingScoiaResolve(action.goFirst);
				player_op.controller.pendingScoiaResolve = null;
			}
			break;
		}
	}
...
function initMultiplayer() {
	if (typeof io === 'undefined') {
		document.getElementById('lobby-status').innerText = "Ошибка: библиотека Socket.io не загружена!";
		alert("Библиотека Socket.io не загружена. Проверьте подключение к сети или запущен ли сервер.");
		return;
	}

	const serverUrl = window.location.protocol === 'file:' ? 'http://localhost:3000' : window.location.origin;
	
	document.getElementById('lobby-status').innerText = "Подключение к серверу...";
	document.getElementById('btn-create-room').style.opacity = "0.5";
	document.getElementById('btn-join-room').style.opacity = "0.5";
	
	socket = io(serverUrl, { reconnectionAttempts: 5, timeout: 5000 });

	socket.on('connect', () => {
		document.getElementById('lobby-status').innerText = "Подключено к серверу. Выберите действие ниже";
		document.getElementById('btn-create-room').style.opacity = "1";
		document.getElementById('btn-join-room').style.opacity = "1";
	});

	socket.on('connect_error', () => {
		document.getElementById('lobby-status').innerText = "Не удалось подключиться. Сервер запущен?";
		document.getElementById('btn-create-room').style.opacity = "0.5";
		document.getElementById('btn-join-room').style.opacity = "0.5";
	});

	socket.on('room_created', (data) => {
		document.getElementById('lobby-initial-actions').classList.add('hide');
		document.getElementById('lobby-waiting').classList.remove('hide');
		document.getElementById('room-display-code').innerText = data.code;
		document.getElementById('lobby-status').innerText = "Комната создана. Поделитесь кодом!";
	});
	...
	socket.on('game_ready', async (data) => {
		multiplayerSeed = data.seed;
		opponentDeckData = data.opDeck;
		myPlayerIndex = data.meIndex;
		...
		setupSeedableRandom(multiplayerSeed);

		player_me = new Player(0, myPlayerIndex === 0 ? "Игрок 1" : "Игрок 2", localDeckData);
		player_op = new Player(1, myPlayerIndex === 0 ? "Игрок 2" : "Игрок 1", opponentDeckData);
		
		player_op.controller = new ControllerNetwork(player_op);
		
		// Мультиплеерный запуск колод в строго фиксированном порядке:
		// Сначала перемешиваем и выкладываем колоду Хоста (Player 1), затем колоду Гостя (Player 2).
		if (myPlayerIndex === 0) {
			player_me.deck.initializeFromID(player_me.deck_data.cards, player_me);
			player_op.deck.initializeFromID(player_op.deck_data.cards, player_op);
		} else {
			player_op.deck.initializeFromID(player_op.deck_data.cards, player_op);
			player_me.deck.initializeFromID(player_me.deck_data.cards, player_me);
		}
	...
	socket.on('opponent_action', async (action) => {
	...
	socket.on('rematch_requested', () => {
		...
		}, "Запрос реванша", "Соперник хочет сыграть еще раз.");
	});
	...
	socket.on('opponent_disconnected', () => {
		if (game.state === GameState.PLAYING && player_me.health > 0 && player_op.health > 0) {
...
document.getElementById('btn-create-room').addEventListener('click', () => {
	if (socket) {
		socket.emit('create_room', localDeckData);
	}
});
...
document.getElementById('btn-join-room').addEventListener('click', () => {
	...
	if (socket) {
		socket.emit('join_room', { code: code, deckData: localDeckData });
	}
});
